...
```{r weigh-in-data}
weight_hx <- 
    googlesheets4::read_sheet("1Gvs_fbePy6xoVDMAaVRJqdWSlD1t8I2MznuhTdELiyA") |> 
    janitor::clean_names() |> 
...
official_race_results_tbl |> 
    filter(
        year(date) == 2024,
        !is.na(official_time)) |> 
    select(date, race, corral, official_time, official_pace, url) |> 
    arrange(desc(date)) |> 
    DT::datatable(
        rownames = FALSE,
        escape = FALSE,
            ...
            columnDefs = list(
                ...
                list(targets = 1, render = DT::JS(
                    "function(data, type, row){
                      if(type === 'display' && row[5] !== null){
                        var a = '<a href=\"' + row[5] + '\">' + data + '</a>';
                        return a;
                      } else {
                        return data;
                      }
